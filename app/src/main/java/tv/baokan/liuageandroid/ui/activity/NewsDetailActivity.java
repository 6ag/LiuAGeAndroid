...
import tv.baokan.liuageandroid.model.CommentBean;
import tv.baokan.liuageandroid.model.UserBean;
import tv.baokan.liuageandroid.adapter.CommentRecyclerViewAdapter;
import tv.baokan.liuageandroid.adapter.LinkRecyclerViewAdapter;
...
public class NewsDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    private SimpleDraweeView mAdImageView;       // 横幅广告
    ...
    private ArticleListBean adArticleBean; // banner广告
    private TextView mAdTextView;          // 广告标题
...
//        mContentWebView.setLayerType(View.LAYER_TYPE_HARDWARE, null);
    ...
    private void prepareData() {
        ...
        if (AdManager.shared().articleListBeanList.size() > 0) {
            ...
            for (ArticleListBean bean :
                    AdManager.shared().articleListBeanList) {
                if (bean.getTitlepic() != null && !bean.getTitlepic().equals("") && bean.getFirsttitle().equals("1")) {
                    tempList.add(bean);
                }
            }
            int randomNum = new Random().nextInt(tempList.size());
            adArticleBean = tempList.get(randomNum);

            // 设置广告图
            mAdImageView.setImageURI(adArticleBean.getTitlepic());
            mAdTextView.setText(adArticleBean.getTitle());
        } else {
            // 无广告时隐藏
    ...
    private void didTappedAdImageView() {
        if (adArticleBean != null) {
            if (adArticleBean.getIsurl().equals("1")) {
                // 打开网页
                AdWebViewActivity.start(mContext, adArticleBean.getTitleurl(), adArticleBean.getTitle(), adArticleBean.getTitlepic());
                return;
            }
            NewsDetailActivity.start(mContext, adArticleBean.getClassid(), adArticleBean.getId());
        }
    }
                ...
                public void onResponse(String response, int id) {
                    ...
                    try {
                        ...
                        if (jsonObject.getString("err_msg").equals("success")) {
                            ...
                            NewsDALManager.shared.removeNewsDetail(classid, NewsDetailActivity.this.id);
                        }
    ...
    private void showCommentDialog() {
        ...
        LinearLayout layout = (LinearLayout) layoutInflater.inflate(R.layout.dialog_comment, null);
        final Dialog commentDialog = new AlertDialog.Builder(this).create();
        commentDialog.show();
        commentDialog.getWindow().setContentView(layout);
        ...
        final EditText commentEditText = (EditText) layout.findViewById(R.id.et_comment_edittext);
        Button cancelButton = (Button) layout.findViewById(R.id.btn_set_font_cancel);
        Button sendButton = (Button) layout.findViewById(R.id.btn_set_font_send);
    ...
    private void sendComment(String comment) {
        ...
        LogUtils.d(TAG, "评论参数 " + parameters.toString());
                ...
                try {
                    ...
                    if (jsonObject.getString("err_msg").equals("success")) {
                        ...
                        if (jsonObject.has("result")) {
                            ProgressHUD.showInfo(mContext, jsonObject.optString("result", "评论失败"));
                        } else {
                            ProgressHUD.showInfo(mContext, "评论失败");
                        }
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
    ...
    private void setupDetailData() {
        ...
        // 1已经收藏过
        if (detailBean.getHavefava().equals("1") && UserBean.isLogin()) {
            mCollectionButton.setImageResource(R.drawable.bottom_bar_collection_selected);
        } else {
            mCollectionButton.setImageResource(R.drawable.bottom_bar_collection_normal2);
        }
        ...
        if (detailBean.getOtherLinks() != null && detailBean.getOtherLinks().size() > 0) {
            ...
            for (ArticleDetailBean.ArticleDetailLinkBean bean :
                    detailBean.getOtherLinks()) {
                LogUtils.d(TAG, bean.toString());
            }

            // 插入广告
            otherLinks.addAll(detailBean.getOtherLinks());
            if (AdManager.shared().articleListBeanList.size() > 0) {
                int randomNum1 = new Random().nextInt(AdManager.shared().articleListBeanList.size());
                int randomNum2 = new Random().nextInt(otherLinks.size());
                ArticleListBean articleListBean = AdManager.shared().articleListBeanList.get(randomNum1);
                ArticleDetailBean.ArticleDetailLinkBean bean = new ArticleDetailBean.ArticleDetailLinkBean(
                        articleListBean.getClassid(),
                        articleListBean.getId(),
                        articleListBean.getOnclick(),
                        articleListBean.getTitle(),
                        articleListBean.getTitlepic(),
                        articleListBean.getTitleurl(),
                        articleListBean.getIsurl(),
                        articleListBean.getClassname());
                otherLinks.add(randomNum2, bean);
            }

            mLinkLayout.setVisibility(View.VISIBLE);
            mLinkRecyclerView.setLayoutManager(new LinearLayoutManager(this));
            mLinkRecyclerViewAdapter = new LinkRecyclerViewAdapter(this, otherLinks);
                    ...
                    FileCacheUtils.checkCacheInDisk(url, new FileCacheUtils.OnCheckCacheInDiskListener() {
                        ...
                        public void checkCacheInDisk(boolean isExist, final String filePath) {
                            if (isExist && !TextUtils.isEmpty(filePath)) {
                                final String sendData = "replaceimage" + url + "~" + filePath;
